...
class PseudoSession {
    ...
    read(nodesToRead, callback) {
        ...
        setImmediate(() => {
            async.map(nodesToRead, (nodeToRead, innerCallback) => {
    ...
    write(nodesToWrite, callback) {
        ...
        const context = session_context_1.SessionContext.defaultContext;
        setImmediate(() => {
            const statusCodesPromises = nodesToWrite.map((nodeToWrite) => {
                node_opcua_assert_1.assert(!!nodeToWrite.nodeId, "expecting a nodeId");
                node_opcua_assert_1.assert(!!nodeToWrite.attributeId, "expecting a attributeId");
                const nodeId = nodeToWrite.nodeId;
                const obj = this.addressSpace.findNode(nodeId);
                if (!obj) {
                    return new node_opcua_data_value_1.DataValue({ statusCode: node_opcua_status_code_1.StatusCodes.BadNodeIdUnknown });
                }
                return util_1.promisify(obj.writeAttribute).call(obj, context, nodeToWrite);
            });
