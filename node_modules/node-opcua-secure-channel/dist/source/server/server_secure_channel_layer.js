...
function dump_request(request, requestId, channelId) {
    console.log(chalk.cyan("xxxx   <<<< ---------------------------------------- "), chalk.yellow(request.schema.name), "requestId", requestId, "channelId=", channelId);
...
class ServerSecureChannelLayer extends events_1.EventEmitter {
    ...
    send_response(msgType, response, message, callback) {
        ...
        /* istanbul ignore next */
        if (doTraceMessage) {
            console.log(chalk.cyan.bold("xxxx   >>>> ---------------------------------------- "), chalk.green.bold(response.schema.name), requestId);
    ...
    _handle_OpenSecureChannelRequest(serviceResult, message, callback) {
        ...
        if (!this.securityHeader) {
            ...
            return this._sendOpenSecureChannelResponseError(node_opcua_status_code_1.StatusCodes.BadCertificateInvalid, message, callback);
        }
        ...
        if (nonceAlreadyBeenUsed(this.clientNonce)) {
            console.log(chalk.red("SERVER with secure connection: Nonee has already been used"), this.clientNonce.toString("hex"));
    ...
    _sendOpenSecureChannelResponseError(serviceResult, message, callback) {
        const response = new services_1.OpenSecureChannelResponse({
            responseHeader: { serviceResult },
            securityToken: this.securityToken,
            serverNonce: this.serverNonce || undefined,
        ...
        this._process_certificates(message, (err, statusCode) => {
            ...
            if (statusCode !== node_opcua_status_code_1.StatusCodes.Good) {
                ...
                if (statusCode === node_opcua_status_code_1.StatusCodes.BadCertificateUntrusted ||
                    statusCode === node_opcua_status_code_1.StatusCodes.BadCertificateRevoked ||
                    false
